...
abstract class AbstractNode(open val configuration: NodeConfiguration,
    ...
    private fun obtainIdentity(notaryConfig: NotaryConfig?): PartyAndCertificate {
        ...
        val (id, singleName) = if (notaryConfig == null) {
            ...
            val notaryId = notaryConfig.run { NotaryService.buildServiceId(validating, raft != null, bftSMaRt != null) }
        ...
        // TODO Should the name of the distributed notary, which the node is part of, be present in the config so that we
        // can cross-check the identity we get from the key store?
        // TODO And if the answer to the above is "no" then should the same logic be applied to myLegalName?
